...
export const POST = apiRoute(async (req: Request) => {
  ...
  if (contentType && !isAllowedUploadContentType(scope, contentType)) {
  ...
  if (validation.warnings.length) {
    logWarn("upload.commit_guardrail_warning", {
      userId: session.user.id,
      sha256,
      key,
      ...buildUploadGuardrailMeta({ scope, contentType, sizeBytes, validation }),
    });
  }
  ...
  if (sizeBytes && head.contentLength && head.contentLength !== sizeBytes) {
    logWarn("upload.commit_size_mismatch", {
      userId: session.user.id,
      sha256,
      key,
      claimedSizeBytes: sizeBytes,
      storedSizeBytes: head.contentLength,
    });
  }
  if (contentType && head.contentType && head.contentType !== contentType) {
    logWarn("upload.commit_content_type_mismatch", {
      userId: session.user.id,
      sha256,
      key,

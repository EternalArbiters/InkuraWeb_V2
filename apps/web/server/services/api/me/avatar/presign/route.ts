...
import { apiRoute, json, unauthorized, badRequest, internalError } from "@/server/http";
...
export const POST = apiRoute(async (req: Request) => {
  ...
  if (!isAllowedUploadContentTypeForScope("avatar", contentType)) return badRequest("Unsupported file type (use PNG/JPG/WebP)");
  const maxBytes = maxBytesForOptimizationScope("avatar");
  if (size && size > maxBytes) return badRequest("File too large (max 2MB)");

  let validation;
  try { validation = validateUploadOptimizationMeta({ scope: "avatar", contentType, sizeBytes: size, meta: optimizationMeta }); }
  catch (error) { return badRequest(error instanceof Error ? error.message : "Invalid avatar optimization metadata"); }

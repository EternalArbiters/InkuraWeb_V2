...
export default function ForgotPasswordPage() {
  const t = useUILanguageText("Page Forgot Password");
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setMessage(
        t("If the account is found, reset-password instructions have been generated. (For production, connect email delivery. If you need the token displayed, set SHOW_RESET_TOKEN=1.)")
      );
      if (json?.resetToken) {
        setToken(String(json.resetToken));
      }
    } catch {
      setMessage(tErrors("An error occurred. Please try again."));
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main className="min-h-screen flex items-center justify-center px-4 bg-gradient-to-br from-[#fdfbff] via-[#f8f5ff] to-[#f4faff] dark:from-[#0a0a1a] dark:via-[#151629] dark:to-[#1b1c34]">
      <div className="w-full max-w-md rounded-2xl border border-white/20 bg-white/80 dark:bg-gray-900/70 backdrop-blur-xl shadow-2xl p-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{t("Forgot password")}</h1>
        <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">{t("Enter your email or username.")}</p>
        ...
        <form onSubmit={onSubmit} className="mt-6 space-y-3">
          <input
            ...
            placeholder={t("Email or username", { section: "Page Sign In" })}
          ...
          >
            {loading ? t("Processing...", { section: "Page Sign In" }) : t("Create reset token")}
          </button>
        </form>
        ...
        {token ? (
          <div className="mt-4 rounded-xl border border-yellow-200 dark:border-yellow-900 bg-yellow-50/70 dark:bg-yellow-950/30 p-3">
            <div className="text-xs font-semibold text-yellow-800 dark:text-yellow-200">{t("Reset token (debug)")}</div>
            <div className="mt-1 text-xs break-all font-mono text-yellow-900 dark:text-yellow-100">{token}</div>
            <div className="mt-2">
              <ActionLink href={`/auth/reset?token=${encodeURIComponent(token)}`}>{t("Continue to password reset")}</ActionLink>
            </div>
          </div>
        ) : null}

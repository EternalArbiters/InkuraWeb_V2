...
export default async function ComicPage() {
  const works = await prisma.work.findMany({
    where: { type: "COMIC", status: "PUBLISHED" },
  ...
  return (
    <main className="min-h-[calc(100vh-96px)] bg-white text-gray-900 dark:bg-gray-950 dark:text-white">
      <div className="max-w-6xl mx-auto px-4 py-10">
        <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight">Comic</h1>
        ...
        <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            >
              ...
              <div className="p-3">
                ...
                <div className="mt-1 text-xs text-gray-600 dark:text-gray-300">
                  oleh {w.author.name || w.author.username}
                </div>
          ...
          {works.length === 0 ? (
            <div className="col-span-2 md:col-span-4 rounded-2xl border border-gray-200 dark:border-gray-800 bg-white/70 dark:bg-gray-900/50 p-6">
              <div className="text-lg font-bold">Belum ada comic</div>
              <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">
                Buat comic baru dari <Link className="text-purple-600 dark:text-purple-400 font-semibold hover:underline" href="/studio/new">Studio</Link>.
              </p>
            </div>
          ) : null}
        </div>

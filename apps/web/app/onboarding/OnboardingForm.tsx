...
export default function OnboardingForm({ initial }: Props) {
  ...
  const t = useUILanguageText("Page Onboarding");
  ...
  const [inkuraLanguage, setInkuraLanguage] = useState<InkuraLanguageCode | "">(initial.inkuraLanguage || "");
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/me/onboarding", {
        ...
        body: JSON.stringify({ gender, birthMonth, birthYear, inkuraLanguage }),
      });
      const payload = await res.json().catch(() => ({}));
      if (!res.ok) {
        throw new Error(typeof payload?.error === "string" ? payload.error : tErrors("Failed to save"));
      }
      await update();
      router.replace("/home");
      router.refresh();
    } catch (err) {
      setError(err instanceof Error ? err.message : tErrors("Failed to save"));
    } finally {
      setSaving(false);
    }
  }
    ...
    <div className="relative overflow-hidden rounded-[32px] border border-white/10 bg-[#09142d] p-6 text-white shadow-2xl shadow-blue-950/30 md:p-8">
      ...
      <div className="relative">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="text-xs font-semibold uppercase tracking-[0.22em] text-blue-200/70">{t("Welcome to Inkura")}</div>
            <h1 className="mt-1 text-2xl font-black tracking-tight md:text-3xl">{t("Finish setting up your account")}</h1>
          </div>
        </div>

        <p className="mt-4 max-w-2xl text-sm leading-6 text-blue-50/80 md:text-base">
          {t("We need your UI language choice before you continue. Demographic details stay here too so analytics and recommendations can understand your audience better.")}
        </p>
        ...
        <form onSubmit={onSubmit} className="mt-8 grid gap-6">
          <label className="block rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur-sm">
            <div className="text-sm font-semibold text-white">{t("What language do you want to use on this website?")}</div>
            <div className="mt-3 grid gap-3 md:grid-cols-2">
              {INKURA_LANGUAGE_OPTIONS.map((option) => {
                const selected = inkuraLanguage === option.code;
                return (
                  <button
                    key={option.code}
                    type="button"
                    onClick={() => setInkuraLanguage(option.code)}
                    className={`rounded-2xl border px-4 py-4 text-left transition ${
                      selected
                        ? "border-blue-400 bg-blue-500/15 text-white shadow-lg shadow-blue-950/20"
                        : "border-white/10 bg-[#0c1a39] text-blue-50/85 hover:border-blue-300/40 hover:bg-white/10"
                    }`}
                    aria-pressed={selected}
                  >
                    <div className="text-base font-bold">{option.label}</div>
                    <div className="mt-1 text-xs text-blue-100/70">{t("Inkura UI language")}</div>
                  </button>
                );
              })}
            </div>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <label className="block rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur-sm">
              <div className="text-sm font-semibold text-white">{tForms("Gender")}</div>
              <select
                value={gender}
            ...
            <label className="block rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur-sm">
              <div className="text-sm font-semibold text-white">{tForms("Birth month")}</div>
              <select
                value={birthMonth}
                onChange={(event) => setBirthMonth(event.target.value ? Number(event.target.value) : "")}
                className="mt-3 w-full rounded-xl border border-white/10 bg-[#0c1a39] px-4 py-3 text-sm text-white outline-none focus:border-blue-400"
              >
                <option value="">{tForms("Select month")}</option>
                {MONTHS.map((option) => (
                  <option key={option.value} value={option.value}>
                    {tForms(option.label)}
                  </option>
          ...
          <label className="block rounded-2xl border border-white/10 bg-white/5 p-4 backdrop-blur-sm">
            <div className="text-sm font-semibold text-white">{tForms("Birth year")}</div>
            <select
              value={birthYear}
              onChange={(event) => setBirthYear(event.target.value ? Number(event.target.value) : "")}
              className="mt-3 w-full rounded-xl border border-white/10 bg-[#0c1a39] px-4 py-3 text-sm text-white outline-none focus:border-blue-400"
            >
              <option value="">{tForms("Select year")}</option>
              {years.map((year) => (
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-xs leading-5 text-blue-100/65">
              {t("You can change your Inkura UI language later from your account settings.")}
            </p>
            <button
              type="submit"
              disabled={saving}
            ...
            >
              {saving ? tForms("Saving...") : t("Continue to Inkura")}
            </button>
          </div>
        </form>
      </div>

      ...
      {works.map((w: any) => {
        ...
        return (
          ...
          >
            ...
            <div className="px-3 pt-1 text-[11px] text-gray-600 dark:text-gray-300">
              {typeof w.chapterCount === "number" ? `${w.chapterCount} ch` : null}
              {w.completion ? ` • ${w.completion}` : ""}
              {w.language ? ` • ${String(w.language).toUpperCase()}` : ""}
            </div>
            <div className="px-3 pt-2 text-[11px] text-gray-600 dark:text-gray-300 flex items-center gap-3">
            ...
            {Array.isArray(w.warningTags) && w.warningTags.length ? (
              <div className="p-3 pt-2 flex flex-wrap gap-1">

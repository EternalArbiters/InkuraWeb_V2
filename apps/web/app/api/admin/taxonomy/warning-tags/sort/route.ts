...
export const POST = apiRoute(async (req: Request) => {
  ...
  try {
    ...
    const rows = await prisma.warningTag.findMany({
      select: {
        id: true,
        name: true,
        isActive: true,
    ...
    const score = (r: (typeof rows)[number]) => (by === "count" ? (r._count.works || 0) + (r._count.chapters || 0) : 0);
    ...
    await prisma.$transaction([
      prisma.$executeRaw(bulkSortOrderUpdateSql('"WarningTag"', pairs)),

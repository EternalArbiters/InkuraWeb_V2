...
export async function POST(req: Request) {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      const rows = await tx.warningTag.findMany({
        select: {
          id: true,
          name: true,
          isActive: true,
          sortOrder: true,
          _count: { select: { works: true, chapters: true } },
        },
        take: 5000,
      });
      ...
      const score = (r: (typeof rows)[number]) =>
        by === "count" ? (r._count.works || 0) + (r._count.chapters || 0) : 0;
      ...
      for (let idx = 0; idx < ordered.length; idx++) {
        ...
        await tx.warningTag.update({ where: { id: r.id }, data: { sortOrder: (idx + 1) * 10 } });
      }

      const afterRows = await tx.warningTag.findMany({
    ...
    console.error("[taxonomy][warning-tags][sort] failed", e);

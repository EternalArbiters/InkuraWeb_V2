...
export const PATCH = apiRoute(async (req: Request, ctx: { params: Promise<{ id: string }> }) => {
  ...
  try {
    const updated = await prisma.$transaction(async (tx) => {
      const before = await tx.deviantLoveTag.findUnique({ where: { id } });
      ...
      const after = await tx.deviantLoveTag.update({ where: { id }, data });
...
export const DELETE = apiRoute(async (req: Request, ctx: { params: Promise<{ id: string }> }) => {
  ...
  try {
    const updated = await prisma.$transaction(async (tx) => {
      ...
      const after = await tx.deviantLoveTag.update({ where: { id }, data: { isActive: false } });
      await tx.adminAuditLog.create({
        data: {
          adminId,
          action: "DEACTIVATE",

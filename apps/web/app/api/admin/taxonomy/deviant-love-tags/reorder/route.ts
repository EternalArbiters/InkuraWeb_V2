...
export async function POST(req: Request) {
  ...
  try {
    const result = await prisma.$transaction(async (tx) => {
      const beforeRows = await tx.deviantLoveTag.findMany({ where: { id: { in: ids } }, select: { id: true, sortOrder: true } });
      const beforeMap = Object.fromEntries(beforeRows.map((r) => [r.id, r.sortOrder]));

      await Promise.all(ids.map((id: string, idx: number) => tx.deviantLoveTag.update({ where: { id }, data: { sortOrder: (idx + 1) * 10 } })));

      const afterRows = await tx.deviantLoveTag.findMany({ where: { id: { in: ids } }, select: { id: true, sortOrder: true } });
